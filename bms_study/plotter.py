...
def construct_s2_trapezoidal_not_achievable(file1: str, file2: str):
    ...
    battery_plot.leg_label = "Working Robots (MILP)"
    battery_plot.color = "#FFA500"
    battery_plot.plot_working()

    battery_plot_2 = BatteryDataPlot(file2)
    battery_plot_2.leg_label = "Working Robots (BCS)"
    battery_plot_2.color = "b"
    battery_plot_2.plot_working()
    #PLT.ylim(26, 40.5)
    PLT.ylim(30, 40.5)
    # PLT.show()
...
def construct_s2_trapezoidal_achievable(file1: str, file2:str):
...
def construct_s1_compare_consumed_energy(file: str) -> None:
    ...
    PLT.bar(
        df[(df["algo_type"] == 1)]["charge_station"] - width / .7,
    ...
    PLT.bar(
        ...
        width,
        label="Modified Rule-Based",
    )

    PLT.bar(
        df[(df["algo_type"] == 2)]["charge_station"] + width / 2,
        df[(df["algo_type"] == 2)]["consumed_charge"] * 40.0,
        width,
        label="Threshold (40-70)",
    )

    # PLT.bar(
    #     df[(df["algo_type"] == 3)]["charge_station"] + width / .7,
    #     df[(df["algo_type"] == 3)]["consumed_charge"] * 40.0,
    #     width,
    #     label="Threshold (30-70)",
    # )

    PLT.bar(
        df[(df["algo_type"] == 4)]["charge_station"] + width / .7,
        df[(df["algo_type"] == 4)]["consumed_charge"] * 40.0,
        width,
        label="Threshold (25-85)",
    )
    ...
    PLT.bar(
        df[(df["algo_type"] == 2)]["charge_station"] + width / 2,
        df[(df["algo_type"] == 2)]["demand_difference_mean"],
        width,
        label="Threshold (40-70)",
    )
    # PLT.bar(
    #     df[(df["algo_type"] == 3)]["charge_station"] + width / .7,
    #     df[(df["algo_type"] == 3)]["demand_difference_mean"],
    #     width,
    #     label="Threshold (30-70)",
    # )
    ...
    PLT.bar(
         df[(df["algo_type"] == 4)]["charge_station"] + width / .7,
         df[(df["algo_type"] == 4)]["demand_difference_mean"],
         width,
         label="Threshold (25-85)",
    )
    ...
    PLT.ylabel("Robot shortfall")
    PLT.legend()
    f.savefig("scenario_1_compare_robot_shortfall.pdf")
...
def main():
    ...
    if plotter_dto.plot_type == PlotType.BAR.value:
        ...
        construct_s2_trapezoidal_achievable(plotter_dto.plot_data_1, plotter_dto.plot_data_2)
    elif plotter_dto.plot_type == PlotType.TRIANGULAR_NOT_ACHIEVABLE.value:
        construct_s2_trapezoidal_not_achievable(plotter_dto.plot_data_1, plotter_dto.plot_data_2)
