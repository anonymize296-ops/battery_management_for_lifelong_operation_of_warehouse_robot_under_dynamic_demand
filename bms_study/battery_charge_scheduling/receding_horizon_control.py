...
class RecedingHorizonControl:
    ...
    def get_horizon_prob(self, t):
        ...
        for k in range(self.horizon):  
            prob_c[k] = self.prob[(t+(k*3)) % len(self.prob)]
            prob_t[k] = self.task_prob[(t+(k*3)) % len(self.task_prob)]
    ...
    def time_model(self, task_prob:list, init_clid, f: TextIOWrapper):
        ...
        """
        task_present = 1 if random.random() < task_prob[0][0] else 0
        self.actual_reward = 1 if task_present else 0

        print("+=============TASK PROB=========")
        print(task_present, task_prob[0][0], task_prob[0])
        # f.write('module time_model\n')
        # f.write(f't:[0..{self.horizon}] init 0;\n')
        # f.write(f'task_present:[0..1] init {task_present};\n')
        # f.write(f'o:[0..1] init 1;\n')
        # f.write(f'e:[0..1] init {task_present};\n')
        f.write('module time_model\n')
        f.write(f't:[0..{self.horizon}] init 0;\n')
    ...
    def cluster_reward(self, f: TextIOWrapper):
        ...
        for e,cl in enumerate(self.clusters):
            f.write(f'[gather_reward] (cl={e}):{cl +1 };\n')
        f.write('[gather_reward] (task_present=1): 1;\n')
        f.write('\nendrewards\n\n')

    def battery_cost_reward(self,f: TextIOWrapper, threshold = 20, threshold2 = 80, cost = 1):
        ...
        for action in self.actions:
            f.write(f'[{action}] (battery > {threshold2}):{cost};\n')
        f.write('\nendrewards\n\n')
